...
`
  ...
  static override examples = ['$ opticloud auth test']

  public async run(): Promise<void> {
    const credentials = await auth.getCredentials()
    if (!credentials) {
      logError('No credentials found. Please run "opticloud auth:login" first.')
      this.exit(1)
    }
    ...
    try {
      ...
      if (validation.valid) {
        ...
        this.log('\\nPlease run "opticloud auth:login" with fresh credentials.')
        this.exit(1)
      }
    } catch (error) {
      spinner.stop()
      ...
      if (error instanceof Error && error.message.includes('No authentication credentials found')) {
        ...
        this.log('\nYour credentials are invalid. Please run "opticloud auth:login" with correct credentials.')
      } else {
