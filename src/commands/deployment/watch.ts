...
`
  ...
  static override examples = [
    ...
    '$ opticloud deployment watch 12345678-1234-1234-1234-123456789012 --poll-interval=30',
    '$ opticloud deployment watch 12345678-1234-1234-1234-123456789012 --project-id=87654321-4321-4321-4321-210987654321',
  ]
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      if (status === 'succeeded' || status === 'failed' || status === 'awaitingverification') {
        ...
        if (status === 'failed') {
          this.log(`Use "opticloud deployment:logs ${deployment.id}" to see error details`)
        } else if (status === 'awaitingverification') {
          this.log(`Use "opticloud deployment:complete ${deployment.id}" to complete the deployment`)
        }
        return
      }
    ...
    while (true) {
      try {
        ...
        if (currentStatus.toLowerCase() === 'succeeded') {
          ...
          this.log(`📋 Use "opticloud deployment:logs ${deploymentId}" for detailed error information`)
          this.log(`🔄 Use "opticloud deployment:reset ${deploymentId}" to reset and retry`)
          break
        } else if (currentStatus.toLowerCase() === 'awaitingverification') {
          this.log('')
          logInfo(`✋ Deployment is awaiting verification`)
          this.log(`✅ Use "opticloud deployment:complete ${deploymentId}" when ready to complete`)
          break
        }
